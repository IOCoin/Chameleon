    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Chameleon address (e.g. ceboWqjtjSCEfwKTy2Hd2Fw2S6HgqsJg96)"));
#endif
...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        cba address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::coinControlChangeChecked(int state)
{
    ...
    {
        if (state == Qt::Checked)
            CoinControlDialog::coinControl->destChange = cba(ui->lineEditCoinControlChange->text().toStdString()).Get();
        else
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        CoinControlDialog::coinControl->destChange = cba(text.toStdString()).Get();

        // label for the change address
        ui->labelCoinControlChangeLabel->setStyleSheet("QLabel{color:black;}");
        if (text.isEmpty())
            ui->labelCoinControlChangeLabel->setText("");
        else if (!cba(text.toStdString()).IsValid())
        {
        ...
        else
        {
            ...
            else
            {
                CPubKey pubkey;
                CKeyID keyid;
                cba(text.toStdString()).GetKeyID(keyid);   
